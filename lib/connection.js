...
module.exports = base.createChild().addInstanceMethods({
  ...
  select: function (coll, criteria, cb) {
    // console.log('select:', coll, criteria);
    var query;
  ...
  insertEach: function (coll, values, cb) {
    var db = this.dbs[coll];
    // console.log("insertEach:", JSON.stringify(values));
    var updated = values.filter(v => v.id || v._id);
    values = values.filter(v => !v.id && !v._id);
    ...
    if(updated && updated.length > 0) {
      ...
      updated.forEach(v => db.update({_id: v.id || v._id}, utils.mapSetter(v), {upsert: true}));
    }
